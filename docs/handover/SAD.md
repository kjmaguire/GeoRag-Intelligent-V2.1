...
- **Settings** (`app/config.py`): Pydantic `BaseSettings` with 135 typed fields. Timeout budgets: `REQUEST_TIMEOUT_S=30`, `AGENTIC_TIMEOUT_S=10`, `TIMEOUT_POSTGIS_S=5`, `TIMEOUT_NEO4J_S=3`, `TIMEOUT_QDRANT_S=2`, `TIMEOUT_RERANKER_S=8`, `TIMEOUT_REDIS_S=0.5`, `TIMEOUT_GATHER_S=8`.
...
- **Service classes** (`app/services/`): auth, persistence (`bronze_store`, `cross_store_consistency`, `cross_workspace_audit`, `trace_writer`), retrieval (`qdrant_service`, `sparse_encoder`, `fusion`, `reranker`, `identifier_boost`, `geological_query_expansion`, `span_resolver`), PDF parse (`pdf_extract`, `pdf_layout`, `pdf_ocr`, `pdf_render`, `pdf_vl`), reasoning (`assessment_summarizer`, `completeness_audit`, `conflict_detector`, `decision_intelligence/`, `report_builder/`, `target_recommendation/`, `target_scoring_ml/`, `geological_ontology/`, `geological_reasoning/`, `llm_incident_diagnosis/`), bridges (`laravel_bridge`, `refusal_builder`, `review_lineage_lookup`, `source_trust/`, `support_cockpit/`, `tool_gateway/`, `visualizations/`, `ingest/`, `eval/`, `shadow_diff/`).

  Removed on the 2026-08-28 dead-code sweep, having had no production caller: `conversation_state_store`, `domain_classifier`, `query_classifier`, `kg_normalizer`, `graph_entity_resolver`, `project_name_resolver`, `publicgeo/`, `pdf_preflight`, `targeting/`. `claim_ledger`, `audit/hash_chain_proof` and `agent/llm_classifier` were deleted 2026-09-07 along with their tests, the orphaned `prompts/classifier_system.py` and the `chaos.yml` workflow whose only test imported the classifier; the `silver.claim_ledger` table stays and is still read by `what_changed_detector` and `answer_runs`. See `scripts/report_dead_modules.py` for the live list. (`seaweedfs_keys`, `answer_run_store` and `citation_lifecycle` were also deleted 2026-09-07 — ADR-0020 limitation 5 for the first; the other two were superseded by the inline persistence in `agentic_retrieval/nodes.py::persist_node`.) Still present with no caller but kept deliberately, per that script's `EXEMPT`: `qdrant_fallback`, `silver_dq_flag_writer`. The `dispatchers/` package (PagerDuty) was deleted 2026-08-28 — it had no caller, no integration key and no account; the escalation that works is the log-marker route through `georag-alerts-ag`.
