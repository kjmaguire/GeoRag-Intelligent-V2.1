...
| ~~Kestra~~ external integration → fastapi | The per-flow JWT machinery (`FLOW_JWT_SECRET`, renamed from `KESTRA_FLOW_JWT_SECRET` on 2026-09-14 per ADR-0022; per-flow private keys in the encrypted `workflow.flow_registry`) is still in `services/flow_jwt.py` and `routers/integrations_trigger.py`, and `flow_jwt_key_reaper` still rotates the keys. The issuer moved `georag-kestra` → `georag-flows` at the same time, which is only a safe change because no token bearing the old issuer was ever minted in production. **Nothing calls it.** | `services/flow_jwt.py` |
