...
| `martin_readonly` | dev: no; Azure: yes | migration `2026_04_22_130000_create_silver_mvt_functions.php` (+ later MVT migrations) | `NOLOGIN NOINHERIT NOSUPERUSER`, EXECUTE on the `silver.pg_*` tile functions. On AWS, Martin connects **as this role**, not as `georag_app` — `deploy/aws/terraform/config.tf` says so twice and hands the task `MARTIN_DATABASE_URL`. This row previously claimed the opposite; the claim was inherited from the Azure tree and was never true of the ECS deployment. `deploy/aws/bootstrap.sql` creates the role with LOGIN before the migration chain runs, because the migration above creates it `NOLOGIN` and nothing else granted it. **Connecting is necessary and not sufficient:** the tile functions are SECURITY INVOKER and set no `app.workspace_id`, so under the tenant policy this role reads zero rows — see appendix C, where that fence is recorded as Open. In compose Martin still connects as `georag_app` (§5). |
