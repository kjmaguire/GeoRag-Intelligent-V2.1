...
# The fixture PDF travels INSIDE the bundle, base64'd.
#
# The 2026-09-18 rehearsal ran the previous bundle twice with the PDF missing
# or the script stale, because "upload two files and put them next to each
# other" is a step that can half-happen. One self-contained artifact cannot.
# ~18 KB of PDF becomes ~24 KB of base64 in a ~36 KB script — nothing next to
# CloudShell's upload limit, and the alternative costs a round trip per
# mistake.
FIXTURE="${HERE}/../../src/fastapi/tests/fixtures/ocr/PLS-2024-Technical-Report.pdf"
if [ ! -f "$FIXTURE" ]; then
  echo "fixture PDF not found at $FIXTURE" >&2
  exit 1
fi
PDF_B64=$(base64 -w0 "$FIXTURE")
...
#    is no embedded_at. ───────────────────────────────────────────────────────
# The fixture PDF, base64. Quoted heredoc: nothing in it is expanded.
read -r -d '' PDF_B64 <<'PDFEOF' || true
${PDF_B64}
PDFEOF
...
case "\$ACTION" in
  ingest)
    # The PDF is carried in this script. PDF=/path/to/other.pdf overrides it.
    #
    # It is deliberately not taken from the container image:
    # docker/fastapi.Dockerfile.dockerignore excludes **/tests/fixtures, so
    # the fixture never reaches the build context and the earlier version of
    # this bundle died with FileNotFoundError trying.
    if [ -n "\${PDF:-}" ]; then
      if [ ! -f "\$PDF" ]; then
      ...
      PDF="\$(mktemp -t rehearsal-XXXXXX.pdf)"
      printf %s "\$PDF_B64" | base64 -d > "\$PDF"
      trap 'rm -f "\$PDF"' EXIT
      echo "using the PDF carried in this script (\$(stat -c%s "\$PDF") bytes)"
    fi
