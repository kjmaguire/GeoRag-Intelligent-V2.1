...
| `laravel-octane-cc` replicas | **still min 1 / max 1** | A second always-on replica was applied 2026-08-23 and reverted the same day on cost (~$60-80/month for 1 vCPU + 2 GiB running 24/7). Nothing technical blocks it, and the objection that did block it is wrong: `max_connections` is 429, the 24h peak was 99, and the second replica moved the observed count by roughly nothing, because Octane opens PDO connections lazily per worker. Session, cache and queue drivers are all redis and the container runs `octane:start` only, so there is no state and no scheduler to double-fire. What 1/1 costs instead: every deploy, node drain and platform restart is a user-visible outage on the only externally-facing app that serves pages. If this is revisited, the floor is the setting that matters — `--max-replicas 2` buys nothing, because ACA only spawns the second replica under load. |
