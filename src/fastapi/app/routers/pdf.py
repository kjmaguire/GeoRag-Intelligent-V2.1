...
def _validate_ocr_bbox(bbox: tuple[float, float, float, float]) -> None:
    ...
    The caller is responsible for supplying a meaningful region bbox. The
    service never falls back to full-page OCR.
    """
    x0, y0, x1, y1 = bbox
    if x0 >= x1 or y0 >= y1:
        raise HTTPException(
...
async def ocr_region(
    ...
    The agent passes an explicit bbox (typically a figure or table region
    whose text was not captured by the pdfminer/pdfplumber Stage 3 path).
