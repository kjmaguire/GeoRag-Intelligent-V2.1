...
COLON-variant DEFAULT task profile (general geological query,
mixed-mode answers).

Consumer: ``app.agent.orchestrator`` — composed prompt (preamble + body).
Activates when ``settings.CITATION_SPAN_RESOLVER_ENABLED=True``.

Text byte-identical to the previous inline ``_SYSTEM_PROMPT_DEFAULT_COLON``.
"""
