"""Colon-variant GRAPH task-profile prompt (knowledge-graph traversal).

⚠️ MIRROR FILE — NOT THE RUNTIME SOURCE OF TRUTH ⚠️
================================================
See ``orchestrator_shared_preamble_dash.py`` for the rationale. The
runtime prompt is the inline ``_SYSTEM_PROMPT_GRAPH_COLON`` constant in
``app/agent/orchestrator.py``.

See ``orchestrator_graph_dash.py`` for the dash-variant drift log — the
same Phase 20 / Phase 22 graft was reverted here on 2026-05-14 so the
file mirrors inline byte-for-byte. The graft is a candidate to
re-introduce alongside the dash version once eval flakiness is fixed.
"""
