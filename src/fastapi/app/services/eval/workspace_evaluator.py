...
NOTE (2026-08-14): restored from the runtime-eval trim (09d1d35). The
`evaluate_workspace` Hatchet workflow that used to wrap
`run_workspace_evaluation()` was deleted in that trim and is NOT
restored — today's only live consumer is
`scripts/run_golden_benchmark.py`, which imports `QuestionRecord`,
`_dsn`, and `_load_active_questions` from this module.
