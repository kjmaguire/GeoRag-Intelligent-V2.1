...
async def _build_agent_deps() -> Any:
    ...
    # B1 (2026-07-28): Neo4j was removed from the stack. AgentDeps accepts
    # neo4j_driver=None and every consumer fails open, so the evaluator no
    # longer constructs a driver (restored 2026-08-14 with this one
    # deliberate deviation from the pre-trim original).
    neo4j_driver = None
    ...
    log.info(
        "real_rag_evaluator.deps_built project_id=%s qdrant=%s neo4j=removed "
        "embedding=%s reranker=%s",
        project_id, qdrant_host,
