...
def test_postgres_object_key_layout() -> None:
    when = datetime(2026, 5, 16, 2, 0, 7, tzinfo=timezone.utc)
...
def test_neo4j_object_key_layout() -> None:
    when = datetime(2026, 5, 16, 2, 15, 30, tzinfo=timezone.utc)
...
def test_object_keys_sort_lexicographically_by_time() -> None:
    ...
    keys = [
        bpg._build_object_key("postgres", rid, datetime(2026, 1, 1, 2, 0, 0, tzinfo=timezone.utc)),
        bpg._build_object_key("postgres", rid, datetime(2026, 1, 1, 14, 0, 0, tzinfo=timezone.utc)),
        bpg._build_object_key("postgres", rid, datetime(2026, 12, 31, 23, 59, 59, tzinfo=timezone.utc)),
        bpg._build_object_key("postgres", rid, datetime(2027, 1, 1, 0, 0, 1, tzinfo=timezone.utc)),
    ]
...
def test_qdrant_object_key_includes_collection() -> None:
    when = datetime(2026, 5, 16, 2, 30, 0, tzinfo=timezone.utc)
...
def test_redis_object_key_uses_rdb_extension() -> None:
    when = datetime(2026, 5, 16, 2, 45, 0, tzinfo=timezone.utc)
...
def test_seaweedfs_snapshot_prefix_layout() -> None:
    when = datetime(2026, 5, 16, 3, 0, 0, tzinfo=timezone.utc)
