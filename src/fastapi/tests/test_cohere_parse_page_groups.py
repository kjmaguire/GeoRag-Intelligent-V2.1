...
class TestSkipEnginePageRequest:
    def test_skips_the_duplicate_billed_request(self) -> None:
        ...
        with patch.object(pdf_report, "_engine_single_page_request") as single, \
            ...
            pdf_report._ocr_single_page(
                "/nonexistent.pdf", 4, skip_engine_page_request=True,
            )
    ...
    def test_an_exhausted_budget_skips_the_engine(self) -> None:
        with patch.object(pdf_report, "_engine_single_page_request") as single, \
             patch.object(pdf_report, "_ocr_budget_take", return_value=False), \
             patch.dict("sys.modules", {"pytesseract": None, "pdf2image": None}):
            out = pdf_report._ocr_single_page(
                "/nonexistent.pdf", 4, return_confidence=True, return_assessment=True,
            )
    ...
    def test_a_non_empty_engine_page_is_returned_with_null_confidence(self) -> None:
        ...
        with patch.object(
            pdf_report, "_engine_single_page_request",
            return_value=_page("Drill hole DDH-1 intersected mineralised quartz.", tables=[grid]),
        ), patch.object(pdf_report, "_ocr_budget_take", return_value=True):
            text, confidence, assessment, tables = pdf_report._ocr_single_page(
                "/nonexistent.pdf", 4,
                return_confidence=True, return_assessment=True, return_tables=True,
            )
    ...
    def test_not_configured_is_critical_not_a_page_failure(self, caplog) -> None:
        import logging
        ...
        assert any(
            r.levelno == logging.CRITICAL and "not configured" in r.getMessage()
...
class TestWholeDocumentGrouping:
    ...
    def test_one_group_per_block_not_per_page(self, stub_page_count, monkeypatch) -> None:
        ...
        def fake_block(_path, first_page, page_count):
            ...
            return {first_page + o: _page(f"PAGE-{first_page + o}") for o in range(page_count)}

        with patch.object(pdf_report, "_ocr_page_block", side_effect=fake_block), \
    ...
    def test_grouped_pages_keep_cohere_parse_provenance(self, stub_page_count) -> None:
        ...
        with patch.object(
            pdf_report, "_ocr_page_block",
            side_effect=lambda _p, first, count: {first + o: _page(f"PAGE-{first + o}") for o in range(count)},
        ), patch.object(pdf_report, "_postprocess_ocr_text", side_effect=lambda t: t):
            result = self._run()

        assert result.parser_used == "ocr_cohere_parse"
        assert all(page.assessment["ocr_method"] == "cohere_parse" for page in result.pages)
        assert all(page.assessment["signals"]["confidence_reported"] is False for page in result.pages)

    def test_empty_page_in_a_good_group_recovers_without_a_second_call(self, stub_page_count) -> None:
        stub_page_count(3)
        seen: dict[int, bool] = {}
    ...
    def test_failed_group_falls_back_to_the_pre_grouping_behaviour(self, stub_page_count) -> None:
        stub_page_count(4)
        seen: dict[int, bool] = {}
        ...
        def fake_single(_path, page_num, **kwargs):
            ...
            return (
                f"PAGE-{page_num}", 0.0,
                pdf_report._assess_ocr_result(f"PAGE-{page_num}", None, detected_region_count=0, ocr_method="cohere_parse"),
                [],
            )

        with patch.object(pdf_report, "_ocr_page_block", return_value={}), \
             patch.object(pdf_report, "_ocr_single_page", side_effect=fake_single), \
             patch.object(pdf_report, "_postprocess_ocr_text", side_effect=lambda t: t):
            result = self._run()
    ...
    def test_batch_size_one_bypasses_the_group_pass_entirely(self, stub_page_count, monkeypatch) -> None:
        ...
        with patch.object(pdf_report, "_ocr_page_block") as block, \
             patch.object(pdf_report, "_ocr_single_page", side_effect=fake_single), \
             patch.object(pdf_report, "_postprocess_ocr_text", side_effect=lambda t: t):
            result = self._run()
...
class TestSelectionAndBudget:
    ...
    def _clean_budget(self):
        for registry in (pdf_report._OCR_PAGES_USED, pdf_report._OCR_CAP_LOGGED, pdf_report._OCR_CAP_EXHAUSTED):
            registry.clear()
        yield
        for registry in (pdf_report._OCR_PAGES_USED, pdf_report._OCR_CAP_LOGGED, pdf_report._OCR_CAP_EXHAUSTED):
            registry.clear()
    ...
    def test_a_block_charges_the_budget_for_every_page_it_covers(self) -> None:
        ...
        with patch.object(
            pdf_report, "_ocr_budget_take", side_effect=lambda _p, pages: taken.append(pages) or True,
    ...
    def test_a_page_outside_the_selection_is_dropped(self) -> None:
        with patch.object(cpc, "ocr_page_block_sync", return_value={3: _page("ok"), 7: _page("?")}):
            out = pdf_report._ocr_page_selection("/tmp/x.pdf", [3, 9])
    ...
    def test_an_unselected_engine_sends_nothing_and_charges_nothing(self, monkeypatch) -> None:
