...
- **Unverified wire shapes**: every model adapter says so at the top, on both hosts. The Foundry path had three behaviours confirmed by a live call on 2026-07-30 (JSON `response_format`, reasoning in a sibling field, Cohere `<|START_TEXT|>`/`<|END_TEXT|>` sentinels) and NONE of them carries over by assumption; Cohere Parse's shape has never been verified on any of the three hosts. The contracts are data — `app/services/bedrock_wire.py` (embed, rerank, the bedrock chat path) and `app/services/cohere_wire.py` (parse) — so the first credentialed run is a diff rather than a discovery. `ops/validation/bedrock_probe.py` still only covers the Bedrock half; **there is no probe for `api.cohere.com` yet**, which is ADR-0023's open verification step.
...
- **The external-LLM egress gate does not cover the primary chat path.** `app/agent/egress_gate.py` is default-deny on `allow_external_llm` and only `_call_anthropic_llm` calls it. With a third-party primary backend, workspace text now leaves the trust boundary on every query without that opt-in. Open SME decision — see ADR-0023 "Consequences". Do not wire the gate onto `llm_cohere.py` without it: the gate is default-deny, so switching it on refuses every query from every workspace that has not set the flag.
