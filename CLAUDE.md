...
- **LLM**: **Amazon Bedrock**, Cohere Command A+ (`command-a-plus-05-2026`; dev + prod). Reached through Bedrock's **Converse** API on a **Marketplace** (SageMaker-managed) endpoint, because Bedrock's serverless Cohere generative catalogue is Command R/R+ (legacy). `LLM_BACKEND` is `bedrock` (default) | `vllm` | `anthropic`; `azure` is a hard startup error naming the replacement. Anthropic Claude (`claude-opus-4-8`, prompt caching on) is wired as optional fallback. Chat lives in `app/agent/llm_bedrock.py`, a sibling of the Anthropic path rather than a branch of the OpenAI-compatible client.
- **Embedding / rerank / OCR**: Bedrock in production, the three compose sidecars in dev (`embedding` Qwen3-Embedding-0.6B and `sparse` SPLADE++ on CPU, `reranker` Qwen3-Reranker-0.6B on the one GPU). `EMBEDDING_BACKEND=bedrock` → Cohere Embed v4 at 1024 dims (matches `georag_chunks`, so no Qdrant migration; switching backends still needs a full re-embed via `scripts/reset_embeddings_for_reencode.py`). `RERANKER_BACKEND=bedrock` → Cohere **Rerank 3.5**, NOT v4 — Bedrock does not serve v4, and `RERANKER_SCORE_THRESHOLD_HOSTED` (0.2, renamed from `_FOUNDRY`) was measured against v4 and **must be re-measured on the golden set**; it is the only retrieval-quality gate in the system. `OCR_ENGINE=cohere_parse` → Cohere Parse 5 on a second Marketplace endpoint. Both variables default to `bedrock` in code and in compose, so an unset value selects the hosted backend rather than a model host that does not exist in production; `.env.example` sets `local` / `cross_encoder` explicitly for the sidecars. Set both identically on the query AND ingest paths — a mismatch writes one vector space and queries another.
- **Unverified wire shapes**: every Bedrock adapter says so at the top. The Foundry path had three behaviours confirmed by a live call on 2026-07-30 (JSON `response_format`, reasoning in a sibling field, Cohere `<|START_TEXT|>`/`<|END_TEXT|>` sentinels) and NONE of them carries over by assumption; Cohere Parse's shape was never verified on either host. Run `ops/validation/bedrock_probe.py` and commit its report before trusting any of them.
- **Marketplace endpoints bill while they exist** — there is no idle state, so the nightly sweeps delete and recreate them. A failed recreate means no chat and no OCR at all, with no invocation metric to alarm on; the startup sweep waits for `InService` and emits `BEDROCK_ENDPOINT_NOT_INSERVICE` otherwise.
