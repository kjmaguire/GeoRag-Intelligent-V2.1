...
class TileProxyController extends Controller
{
    ...
    private const SILVER_SOURCES = [
        ...
        // Added on restore, 2026-08-25. Both functions were created while
        // this proxy was deleted (0eada56c removed the service; the
        // migrations kept going), so they were never in the whitelist:
        //   silver.pg_spatial_features_by_project  2026-08-23
        //   silver.pg_cross_section_lines_by_project
        // spatial_features is where every shapefile, GeoPackage, DXF and
        // Surpac string lands, i.e. the layer most of an uploaded delivery
        // actually becomes. Both verified live on the Azure server with
        // EXECUTE granted to martin_readonly.
        'pg_spatial_features_by_project',
    ...
    private function computePgeoEtag(int $z, int $x, int $y): string
    {
        ...
        $epochS = Cache::remember(
            ...
            function (): int {
                ...
                // `$row` is never null — this is an aggregate, so MAX() over
                // an empty table still returns one row. `epoch_s` IS null in
                // that case, which is what the ?? covers.
