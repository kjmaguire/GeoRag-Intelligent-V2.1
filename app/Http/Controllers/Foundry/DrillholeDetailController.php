 ...
 * original `fetchVisualQa()` call to FastAPI `POST /v1/viz/qa` — that
 * endpoint was itself deleted the day after this controller was (confirmed
 * dead: nothing in Laravel or the frontend called it once this controller
 * was gone), so restoring the call would just add a permanently-failing
 * request wrapped in a try/catch. No `qa` prop is passed to the page
 * anymore; the QA panel was removed from DrillholeDetail.tsx accordingly.
 */
class DrillholeDetailController extends Controller
{
    ...
    public function show(Request $request, string $slug, string $collarId): Response
    {
        ...
        return $this->withWorkspaceRls($workspaceId, function () use ($project, $collarId) {
