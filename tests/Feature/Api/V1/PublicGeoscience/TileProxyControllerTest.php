 ...
 *
 * Postgres-only. The ETag path derives a jurisdiction epoch with
 * `EXTRACT(EPOCH FROM MAX(updated_at))::bigint`, which is a syntax error under
 * SQLite — so on the default connection six of these fail on the driver rather
 * than on the behaviour they test. Restored 2026-08-25 alongside Martin; the
 * suite predates RequiresPostgres, which is why it carried no guard.
 */
